function drawTree(input, w, h) {
	var json = traverseTree(input, '')
	createTree(json, w, h)
}

function traverseTree(input, val) {
	var json = {}
	if (input.cutoffValue) {
		return {'name': (val + 'Is ' + input.cutoffType + ' ' + input.cutoffName + ' ' + input.cutoffValue.toFixed(2) + '?'), 
			'children': [traverseTree(input.results.isTrue, 'T, '), traverseTree(input.results.isFalse, 'F, ')]};
	} else {
		return  {'name': (val + input.results)}
	}
}
