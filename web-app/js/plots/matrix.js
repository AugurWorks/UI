...
function setMatrix(matrixData, mat, mar) {
	...
	var svg = d3.select("#matrix").append("svg")
	  ...
	  row.append("text")
	      ...
	      .text(function(d, i) { return nodes[i].name.split('-')[0]; });
	  ...
	  function row(row) {
	    var cell = d3.select(this).selectAll(".cell")
	        ...
	        .style("fill-opacity", function(d) { return mat ? z(d.z) : Math.abs(d.z); })
	        .style("fill", function(d) { return mat ? (nodes[d.x].group == nodes[d.y].group ? c(nodes[d.x].group) : null) : (d.z < 0 ? 'red' : '#0098D9'); })
