	...
	def init = { servletContext ->
		try {
			if (Role.count() == 0) {
				...
				def tips = new DataType(name:'Treasury Inflation Protected Rates', valueType:'Number', optionNum: 2, serviceName: 'quandl', label: 'Treasury Rates', unit: '%').save()
				tips.addToDataChoices(name: '5-Year Treasury Inflation-Indexed Security', key: 'FRED/DFII5.json?')
				tips.addToDataChoices(name: '7-Year Treasury Inflation-Indexed Security', key: 'FRED/DFII7.json?')
				tips.addToDataChoices(name: '10-Year Treasury Inflation-Indexed Security', key: 'FRED/DFII10.json?')
				tips.addToDataChoices(name: '20-Year Treasury Inflation-Indexed Security', key: 'FRED/DFII20.json?')
				tips.addToDataChoices(name: '30-Year Treasury Inflation-Indexed Security', key: 'FRED/DFII30.json?')
				def dep = new DataType(name:'Deposit Rates', valueType:'Number', optionNum: 3, serviceName: 'quandl', label: 'Deposit Rates', unit: '%').save()
