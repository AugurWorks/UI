...
<body>
	...
	<script type="text/javascript">
		...
		function formatAllData(dataSet) {
			var obj = {'nodes': [], 'links': []}
			...
			for (i in dataSet) {
				if (dataSet[i].entities != undefined) {
					for (var j = 0; j < dataSet[i].entities.length; j++) {
						if (names.indexOf(dataSet[i].entities[j].actual_name) == -1) {
			...
			var cut = vals.sort().reverse()[cutNum]
			names = []
			
			for (i in dataSet) {
				if (dataSet[i].entities != undefined) {
					for (var j = 0; j < dataSet[i].entities.length; j++) {
						if ((dataSet[i].entities[j].averageFreq >= cut && obj['nodes'].length < cutNum) || names.indexOf(dataSet[i].entities[j].actual_name) != -1) {
							var nodes = obj['nodes']
							var src = names.indexOf(dataSet[i].entities[j].actual_name)
							if (src == -1) {
								src = nodes.length
								nodes.push({'name': dataSet[i].entities[j].actual_name, 'group': dataSet[i].entities[j].type})
								names.push(dataSet[i].entities[j].actual_name)
							}
							for (var k = 0; k < j; k++) {
								var tar = names.indexOf(dataSet[i].entities[k].actual_name)
								if (tar != -1) {
		...
		function setMatrix() {
			var matrixData = formatAllData(dataSet)
